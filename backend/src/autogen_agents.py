...
def build_director_system_message(
    ...
    task_info = (
        ...
        + "\n\n"
        + "2. **走位设计（以演出效果为唯一标准）**:\n"
        + "   - 根据演出需要决定角色站位，在 position 字段用一句自然语言描述站位的空间语义和戏剧意图\n"
        + "   - 例：\"position\": \"近窗俯瞰，背靠星空，适合独白或凝望\"\n"
        + "   - 例：\"position\": \"门口，适合等候与守卫\"\n"
        + "   - 无需编写 position_descriptions 字段，直接在 position 字段写描述\n"
        + "   - 位置专家会根据你的描述选择最合适的真实点位，你只需专注演出效果\n\n"
        + "3. **动作决策**:\n"
        + "   - 只能使用\"可用动作库\"中的动作名称\n"
        ...
        + "   - 对白要符合人物性格和场景氛围\n\n"
        + "5. **镜头设计**:\n"
        + "   - 所有片段（对白、旁白、移动）的 `shot` 字段统一填 `\"character\"`\n"
        + "   - 使用 `shot_anchors` 数组指定镜头朝向，从 `\"Left\"`、`\"Right\"`、`\"Front\"` 中选择\n"
        + "   - 根据场景氛围和说话者位置选择合适朝向，例如：正面对话用 `[\"Front\"]`，侧面对话用 `[\"Left\"]` 或 `[\"Right\"]`\n\n"
        ...
        + "    },\n"
        + "    \"initial position\": [\n"
        + "      {\"character\": \"角色名1\", \"position\": \"舱体中央，适合面对面对峙\"}\n"
        + "    ],\n"
        + "    \"scene\": [\n"
        + "      {\n"
        + "        \"speaker\": \"角色名\",\n"
        ...
        + "      },\n"
        + "      {\n"
        + "        \"move\": [{\"character\": \"角色名\", \"destination\": \"走廊深处，适合独自离开\"}],\n"
        + "        \"shot\": \"character\",\n"
        + "        \"shot_anchors\": [\"Front\"],\n"
        + "        \"actions\": [\n"
        + "          {\"character\": \"角色名\", \"state\": \"standing\", \"action\": \"Standing Walking\", \"motion_detail\": \"\"}\n"
        + "        ],\n"
        + "        \"current position\": [\n"
        + "          {\"character\": \"角色名1\", \"position\": \"舱体中央，适合面对面对峙\"}\n"
        + "        ]\n"
        ...
        + "```\n\n"
        + "**字段规则:**\n"
        + "- `shot` 所有片段统一填 `\"character\"`，不使用 `\"scene\"` 和 `camera` 字段\n"
        + "- `shot_anchors`：每个片段必填，数组，从 `\"Left\"`、`\"Right\"`、`\"Front\"` 中选一个\n"
        + "- **`current position` 是每个片段（对白、旁白、移动）的强制必填字段，绝对不能省略。**\n"
        + "  每个片段必须列出场景内所有在场角色当前所在的位置描述。\n"
        + "  移动片段的 `current position` 记录的是移动*前*的位置。\n"
        + "- `position` 字段只写自然语言描述，不要写 Position 1/2/3 等编号，位置专家会负责匹配真实点位\n"
        + "- 只使用可用动作库中的动作名称\n"
    )
...
def build_position_agent_system_message(scene: Scene) -> str:
    ...
    return (
        "你是位置映射专家。剧本中的 position 字段包含自然语言描述（由导演编写的演出意图），"
        "你的任务是将每处描述替换为下方列出的真实点位 ID。\n\n"
        f"## 当前场景：{scene.name} (ID: {scene.id})\n\n"
        "### 可用真实点位:\n"
        + positions_info
        + camera_groups_info
        + "\n\n## 强制规则（违反即输出无效）:\n\n"
        "1. 只能使用上方列出的点位 ID，绝对不得创造不存在的 ID\n"
        "2. 非移动片段（无 `move` 字段的片段）中，所有角色的点位必须属于同一镜头组\n"
        "3. 若某描述无完美匹配，在同一镜头组内选功能最接近的点位，而非跨组\n\n"
        "## 工作步骤:\n\n"
        "1. 读取剧本中所有出现过的自然语言位置描述（`initial position`、`current position`、`move.destination`）\n"
        "2. 为每个唯一描述选择最匹配的真实点位 ID\n"
        "3. 确保同一非移动片段内所有角色的点位属于同一镜头组\n"
        "4. 将剧本中所有 position 字段的自然语言替换为真实点位 ID\n"
        "5. 输出修改后的完整剧本 JSON\n\n"
        "**直接输出 JSON，无需额外说明。**"
    )
